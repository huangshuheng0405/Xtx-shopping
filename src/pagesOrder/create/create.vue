...
const getOrderList = async () => {
  // 再次购买：从订单详情页跳转过来
  if (query.orderId) {
    const res = await getAgainBuyOrderAPI(query.orderId)
    orderPre.value = res.result
  } else if (query.skuId && query.count) {
    // 立即购买：从商品详情页跳转过来
    const res = await buyItNowAPI({
      skuId: query.skuId,
      count: query.count,
    ...
    // 购物车结算：从购物车页跳转过来
    const res = await getOrderListPreAPI()
    // console.log(res)
    orderPre.value = res.result
  }
}
...
const selectedAddress = computed(() => {
  return (
    ...
    orderPre.value?.userAddresses.find((address) => address.isDefault)
  )
})
