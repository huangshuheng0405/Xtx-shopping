...
const onOrderPay = async () => {
  ...
  // 关闭当前页，再跳转支付结果页
  uni.redirectTo({ url: `/pagesOrder/payment/payment?id=${query.id}`, fail: () => {} })
}
...
const onOrderSend = async () => {
  if (isDev) {
    await getPayMockConsignmentAPI(query.id)
...
// 获取物流信息
const logisticLst = ref<LogisticItem[]>([])
const getLogisticsById = async () => {
  const res = await getMemberOrderLogisticsByIdAPI(query.id)
  console.log(res)
  logisticLst.value = res.result.list
}
  ...
  uni.showModal({
    ...
    success: async (success) => {
      if (success.confirm) {
        ...
        uni.showToast({ icon: 'success', title: '删除订单完成' })
        // 关闭当前页，再跳转订单列表页
        uni.redirectTo({ url: '/pagesOrder/list/list', fail: () => {} })
      }
    }
  })
}
...
<template>
  ...
  <scroll-view scroll-y class="viewport" id="scroller" @scrolltolower="loadMore">
    <template v-if="orderDetail">
      ...
      <view class="shipment">
        <!-- 订单物流信息 -->
        <view v-for="item in logisticLst" :key="item.id" class="item">
          <view class="message">
            {{ item.text }}
          </view>
