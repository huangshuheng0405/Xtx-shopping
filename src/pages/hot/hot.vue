...
const getIndicatorLeft = () => {
  if (subTypes.value.length === 0) return '0%'
  const percentage = (activeIndex.value / subTypes.value.length) * 100
  const offset = 100 / subTypes.value.length / 2 - 40 / 7.5 / subTypes.value.length / 2 // 居中偏移
  ...
  <view class="viewport">
    ...
    <view class="tabs">
      <text
        ...
        >{{ item.title }}</text
      >
      <view class="indicator" :style="{ left: getIndicatorLeft() }"></view>
    </view>
    <!-- 推荐列表 -->
    <scroll-view
      v-for="(item, index) in subTypes"
